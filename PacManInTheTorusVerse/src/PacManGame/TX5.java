...
public class TX5 extends Monster {
    ...
    /**
     * Similar to walkApproach, but TX5 moves 2 cells instead of 1
     */
    @Override
    public void angrywalkApproach(){
        ...
        Location.CompassDirection compassDir =
                getLocation().get4CompassDirectionTo(pacLocation);
        Location interum = getLocation().getNeighbourLocation(compassDir);
        setDirection(compassDir);
        Location next = interum.getNeighbourLocation(getDirection());

        if (!isVisited(next) && canMove(next) && canMove(interum)) {
            setLocation(next);
        }
        else {
            int sign = getRandomiser().nextDouble() < 0.5 ? 1 : -1;
